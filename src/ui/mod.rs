...
//!    has run past `SHOW_AFTER`; api.cloudflare.com usually answers first.
...
pub fn init(quiet: bool) {
    let silenced = quiet || env_flag("MLAB_CLOUDFLARE_NO_PROGRESS");
...
/// A blank line on stderr, so a block of observations does not run into the
/// count line above it. Emitted on stderr rather than stdout because that is
/// where the observations themselves go, and the two streams only interleave
/// predictably when each spaces its own output.
pub fn gap() {
    if quiet() {
        return;
    }
